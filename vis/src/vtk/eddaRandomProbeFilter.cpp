/*=========================================================================
  ...
  Module:    eddaRandomProbeFilter.cxx
...
=========================================================================*/
#include "eddaRandomProbeFilter.h"
...
vtkStandardNewMacro(eddaRandomProbeFilter)

using namespace edda;
using namespace std;

class eddaRandomProbeFilter::vtkVectorOfArrays :
  public std::vector<vtkDataArray*>
{
};

//----------------------------------------------------------------------------
eddaRandomProbeFilter::eddaRandomProbeFilter()
{
  this->SpatialMatch = 0;
...
//----------------------------------------------------------------------------
void eddaRandomProbeFilter::SetSourceConnection(vtkAlgorithmOutput* algOutput)
{
  this->SetInputConnection(1, algOutput);
}

//----------------------------------------------------------------------------
void eddaRandomProbeFilter::SetSourceData(vtkDataObject *input)
{
  this->SetInputData(1, input);
}

//----------------------------------------------------------------------------
vtkDataObject *eddaRandomProbeFilter::GetSource()
{
...
//----------------------------------------------------------------------------
int eddaRandomProbeFilter::RequestData(
...
//----------------------------------------------------------------------------
void eddaRandomProbeFilter::PassAttributeData(
...
//----------------------------------------------------------------------------
void eddaRandomProbeFilter::BuildFieldList(vtkDataSet* source)
{
  delete this->PointList;
  delete this->CellList;
...
// * output - output.
void eddaRandomProbeFilter::InitializeForProbing(vtkDataSet* input,
  vtkDataSet* output)
{
...
//----------------------------------------------------------------------------
void eddaRandomProbeFilter::Probe(vtkDataSet *input, vtkDataSet *source,
                           vtkDataSet *output)
{
  bool flag = false;
  // Use random sampling array
  GmmVtkDataArray *gmmDataArray = new GmmVtkDataArray(source->GetPointData());
  if (gmmDataArray->getLength() > 0) {
    vsp_new( eddaSamplingArray, samplingDataArray);
    samplingDataArray->SetEddaArray(gmmDataArray);
    samplingDataArray->SetName("RandomSample");
    source->GetPointData()->Reset();
    source->GetPointData()->AddArray(samplingDataArray);
    flag = true;
  } else
    delete gmmDataArray;

  GmmVtkDataArray *gmmCellArray = new GmmVtkDataArray(source->GetCellData());
  if (gmmCellArray->getLength() > 0) {
    vsp_new( eddaSamplingArray, samplingCellArray);
    samplingCellArray->SetEddaArray(gmmCellArray);
    samplingCellArray->SetName("RandomSample");
    source->GetCellData()->Reset();
    source->GetCellData()->AddArray(samplingCellArray);
    flag = true;
  } else
    delete gmmCellArray;

  if (!flag)
    printf("No GMM arrays found\n");
...
//----------------------------------------------------------------------------
void eddaRandomProbeFilter::ProbeEmptyPoints(vtkDataSet *input,
  int srcIdx,
...
//----------------------------------------------------------------------------
int eddaRandomProbeFilter::RequestInformation(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **inputVector,
  vtkInformationVector *outputVector)
{
...
//----------------------------------------------------------------------------
int eddaRandomProbeFilter::RequestUpdateExtent(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **inputVector,
  vtkInformationVector *outputVector)
{
...
//----------------------------------------------------------------------------
void eddaRandomProbeFilter::PrintSelf(ostream& os, vtkIndent indent)
{
