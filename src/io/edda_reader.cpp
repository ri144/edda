...
namespace edda {
	...
	/// Historical non-OO function to be deprecated. Instead, use the class EddaReader
	...
	std::shared_ptr<Dataset<Real> > EddaReader::loadEddaDataset(const std::string &edda_file)
	{
		ifstream myfile(edda_file.c_str(), ios::binary);
		...
		if (majorVersion == 0 && minorVersion == 1) {
			...
			*/
			...
			if (gridTypeNumber == 1) {
				...
				return make_shared<Dataset<Real>>(new RegularCartesianGrid(dims[0], dims[1], dims[2]), dVec);
			}
			else {
				myfile.close();
				throw NotImplementedException();
	...
	DistrArray * EddaReader::readMixArray(ifstream & myfile, int n)
	{
		dist::Variant* distArray;
		distArray = new dist::Variant[n];
