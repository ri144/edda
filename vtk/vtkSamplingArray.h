#ifndef VTK_SAMPLING_ARRAY_H
#define VTK_SAMPLING_ARRAY_H
#include <cstdio>
...
class vtkSamplingArray : public vtkDataArray
{
  ...
  double tmp[3];
public:
  vtkTypeMacro(vtkSamplingArray,vtkDataArray)
  static vtkSamplingArray *New();
  ...
  {
    VECTOR3 v = boost::any_cast<VECTOR3>( pArray->getItem(i) );
    tmp[0] = v[0];
    tmp[1] = v[1];
    tmp[2] = v[2];
    return tmp;
  ...
  //------------------------------------------------------------------------------
  inline static vtkSamplingArray* FastDownCast(vtkAbstractArray *source)
  {
    switch (source->GetArrayType())
      ...
      case MappedDataArray:
        return static_cast<vtkSamplingArray*>(source);
      default:
        return NULL;
      }
  }
...
private:
  vtkSamplingArray(const vtkSamplingArray& a);  // Not implemented.
  void operator=(const vtkSamplingArray& a);  // Not implemented.
};
